...
def create_pipeline():
    ...
    # All works with 2020-4 models
    detection_nn = pipeline.createMobileNetDetectionNetwork()
    #detection_nn.setBlobPath(str(Path("E:/OpenVINO-Re-ID/ver-2020-4/yolo-v2-ava-0001.blob").resolve().absolute()))
    #detection_nn.setBlobPath(str(Path("E:/OpenVINO-Re-ID/ver-2020-4/yolo-v3-tf.blob").resolve().absolute()))
    detection_nn.setBlobPath(str(Path("E:/OpenVINO-Re-ID/ver-2020-4/yolo-v2-tiny-vehicle-detection-0001-2020-4.blob").resolve().absolute()))
    #detection_nn.setBlobPath(str(Path("E:/OpenVINO-Re-ID/ver-2020-4/vehicle-detection-adas-0002.blob").resolve().absolute()))
    #detection_nn.setBlobPath(str(Path("models/person-detection-retail-0013_openvino_2020.1_4shave.blob").resolve().absolute()))
    #detection_nn.setBlobPath(str(Path("E:/OpenVINO-Re-ID/ver-2020-4/pedestrian-and-vehicle-detector-adas-0001.blob").resolve().absolute()))
    # Confidence
    ...
    # works with the 2021.1
    #reid_nn.setBlobPath(str(Path("models/person-reidentification-retail-0031_openvino_2020.1_4shave.blob").resolve().absolute()))
    #reid_nn.setBlobPath(str(Path("E:/OpenVINO-Re-ID/ver-2020-4/person-reidentification-retail-0031_openvino_2020.1_4shave.blob").resolve().absolute()))

    # Dosen't work with 2021-3
    #reid_nn.setBlobPath(str(Path("E:/OpenVINO-Re-ID/ver-2021-3/vehicle-reid-0001.blob").resolve().absolute()))  # Vehicle RE-ID
    # Dosen't work with 2020-4
    reid_nn.setBlobPath(str(Path("E:/OpenVINO-Re-ID/ver-2020-4/vehicle-reid-0001.blob").resolve().absolute()))  # Vehicle RE-ID
    # Dosen't work with 2021-4
    #reid_nn.setBlobPath(str(Path("E:/OpenVINO-Re-ID/ver-2021-1/vehicle-reid-0001.blob").resolve().absolute()))  # Vehicle RE-ID
    # Dosen't work with 2021-4
    #reid_nn.setBlobPath(str(Path("E:/OpenVINO-Re-ID/ver-2021-4/vehicle-reid-0001-2021-4.blob").resolve().absolute()))  # Vehicle RE-ID
        ...
        while self.running:
            try:
                ...
                for det in inference.detections:
                    ...
                    #deep_sort
                    det_bboxes[noVehicles] = bbox #not raw
                    feature_cosine_ndarray[noVehicles] = reid_result
                    scores.append(0.7)
                    names.append('cars')
                    noVehicles += 1
                ...
                if debug:
                    ...
                    counter =0
                    noVehicles = len(bboxes)
                    #arrbboxes = np.array(bboxes)
                    #deepsort
                    arrbboxes = utils.format_boxes(det_bboxes, 1, 1)
                    ...
                    print("# of tracker-detections after non-maxima:", len(inference.detections), len(detections))
                    '''
                    tracker.predict()
                    tracker.update(detections)

                    
                    for vehicle in range(noVehicles):
                        for frame in frames:
                           cv2.rectangle(frame, (bboxes[vehicle][0], bboxes[vehicle][1]), (bboxes[vehicle][2], bboxes[vehicle][3]), (10, 245, 10), 2)
                           #cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (10, 245, 10), 2)
                           #x = (bbox[0] + bbox[2]) // 2
                           #y = (bbox[1] + bbox[3]) // 2
                           print("frames#",counter, (bboxes[vehicle][0], bboxes[vehicle][1]), (bboxes[vehicle][2], bboxes[vehicle][3]))
                           counter +=1
                    ...
                    for track in tracker.tracks:
                        for frame in frames:                    
                            ...
                            print("reid_tracker_bbox",reid_tracker_bbox)

                            # draw RED bbox on screen - tracker cordinates swapped
                            color = (10, 10, 245)
                            cv2.rectangle(frame, (int(reid_tracker_bbox[1]), int(reid_tracker_bbox[0])), (int(reid_tracker_bbox[3]), int(reid_tracker_bbox[2])), color, 2)
                            cv2.rectangle(frame, (int(reid_tracker_bbox[1]), int(reid_tracker_bbox[0] - 30)),
                                   (int(reid_tracker_bbox[1]) + (len(class_name) + len(str(track.track_id))) * 17, int(reid_tracker_bbox[0])),
                                           color, -1)
                            cv2.putText(frame, class_name + "-" + str(track.track_id), (int(reid_tracker_bbox[1]), int(reid_tracker_bbox[0] - 10)), 0,
                                           0.75, (255, 255, 255), 2)
